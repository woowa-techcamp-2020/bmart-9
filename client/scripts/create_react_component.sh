...
# index.ts
echo `echo "export {default as ${FILE_NAME}} from './${FILE_NAME}${FIRST_CAP}'" > src/${FOLDER}/${FILE_NAME}/index.ts`
...
const ${FILE_NAME}${FIRST_CAP}: React.FC<Props> = ({} : Props) => {

	return <S.Container>${FILE_NAME}</S.Container>;
};

export default ${FILE_NAME}${FIRST_CAP};
" > src/${FOLDER}/${FILE_NAME}/${FILE_NAME}${FIRST_CAP}.tsx`
