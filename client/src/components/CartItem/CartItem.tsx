...
const CartItem: React.FC<Props> = ({id,checked:initChecked,name,img,base_price,discount,price} : Props) => {
	...
	let description:string[] = ["","1개 이하로 선택할 수 없습니다.","해당 상품은 한번에 10개까지 구매할 수 있어요."]
	...
	return <S.Container>
		...
		<S.cartBody>
			...
			<S.cartPriceWrapper>
				...
				<div>
					<S.cartTotalBasePrice>{comma(base_price*quantity)}원</S.cartTotalBasePrice>
				...
				<S.cartQuantityWrapper>
					<S.cartQuantityMinus onClick={() => decreaseQuantity()}> - </S.cartQuantityMinus>
					<S.cartQuantity>{quantity}</S.cartQuantity>
					<S.cartQuantityPlus onClick={() => increaseQuantity()}> + </S.cartQuantityPlus>
				</S.cartQuantityWrapper>
			</S.cartPriceWrapper>
		</S.cartBody>
	</S.Container>;
